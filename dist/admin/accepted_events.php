...
$sql = "SELECT * FROM bookings WHERE status = 'ACCEPT' ORDER BY event_date ASC, event_time ASC";
$bookings = $conn->query($sql);

$count_result = $conn->query("SELECT COUNT(*) as count FROM bookings WHERE status = 'ACCEPT'");
$total_accepted = $count_result->fetch_assoc()['count'];
?>
<!doctype html>
...
<head>
    ...
    <link rel="stylesheet" href="../assets/css/css_darkmode/accepted_dn.css" id="main-style-link" />
    <link rel="stylesheet" href="../assets/css/responsive/pending.css" id="main-style-link" />
    <link rel="stylesheet" href="../assets/css/main/accepted.css" id="main-style-link" />
</head>
...
<!-- [ Header ] end -->
    ...
    <div class="ml-[264px] p-10 max-w-[1400px]">
        <!-- Page Header -->
        <div class="mt-[50px] page-header bg-gradient-to-r from-[#27ae60] to-[#229954] text-black p-10 rounded-[20px] mb-9 shadow-xl relative overflow-hidden">
            <h2 class="m-0 mb-3 text-[2.5rem] font-black text-black relative z-10">Accepted Events</h2>
            <p class="m-0 text-[1.1rem] opacity-90 font-medium text-black relative z-10">
                Total Accepted Reservations: <strong><?php echo $total_accepted; ?></strong>
            </p>
        </div>
        ...
        ?>
        <div class="booking-table bg-white rounded-[20px] shadow-lg mb-8 overflow-hidden border-l-5 border-[#27ae60] transition-all duration-300 hover:translate-y-[-5px] hover:shadow-xl">
            <!-- Table Header -->
            <div class="table-header bg-gradient-to-r from-[#d4edda] to-[#c3e6cb] text-black p-5 px-8 font-bold text-[1.1rem] flex justify-between items-center border-b-3 border-[#28a745]">
                <span>BOOKING ID: #<?php echo $row['booking_id']; ?></span>
                <span class="status-badge px-5 py-[10px] rounded-[25px] font-bold text-[0.9rem] bg-gradient-to-r from-[#d4edda] to-[#c3e6cb] text-black border-2 border-[#28a745] shadow-lg uppercase tracking-[0.5px]">
                    ACCEPTED
                </span>
            </div>
            ...
            <div class="info-section p-8 border-b-2 border-[#f5f5f5]">
                ...
                <div class="info-grid grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                    <div class="info-item flex flex-col gap-2 p-4 bg-[#fafafa] rounded-[10px] transition-all duration-300 hover:bg-[#e8f5e9] hover:translate-y-[-2px]">
                    ...
                    <div class="info-item flex flex-col gap-2 p-4 bg-[#fafafa] rounded-[10px] transition-all duration-300 hover:bg-[#e8f5e9] hover:translate-y-[-2px] md:col-span-3">
                    ...
                    <div class="info-item flex flex-col gap-2 p-4 bg-[#fafafa] rounded-[10px] transition-all duration-300 hover:bg-[#e8f5e9] hover:translate-y-[-2px]">
                        <span class="info-label text-[0.85rem] text-[#666] font-semibold uppercase tracking-[0.5px]">Accepted At</span>
            ...
            <div class="booking-details p-8 bg-gradient-to-r from-[#fafafa] to-[#f5f5f5]">
                ...
                <div class="details-grid flex flex-col gap-4">
                    ...
                    <div class="detail-item bg-white p-6 rounded-[12px] border-l-5 border-[#27ae60] shadow-md transition-all duration-300 hover:translate-y-[-3px] hover:shadow-lg flex flex-col">
                    ...
                    <div class="detail-item bg-white p-6 rounded-[12px] border-l-5 border-[#27ae60] shadow-md transition-all duration-300 hover:translate-y-[-3px] hover:shadow-lg flex flex-col">
                        ...
                        <div class="detail-value text-[2.2rem] font-black text-center p-3 border-3 border-[#e8f5e8] rounded-[12px] mt-3">
        ...
        <div class="empty-state text-center py-20 px-10 bg-white rounded-[20px] shadow-lg relative overflow-hidden">
            <h3 class="m-0 mb-4 text-[1.8rem] text-black font-bold">No Accepted Events</h3>
            <p class="text-[1.1rem] text-[#666] leading-[1.6]">No bookings have been accepted yet.</p>
        </div>
        <?php } ?>
    </div>
    <!-- [ Main ] end -->
